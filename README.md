...
<a href="https://www.codecogs.com/eqnedit.php?latex=\sum_{w\inV}\sum_{c\inV}\left&space;(&space;\&hash;(w,c)&space;\cdot&space;log(\sigma(sim(w,c)))&plus;k\cdot&space;\&hash;(w\cdot&space;P_0(c)\cdot&space;log(\sigma(-sim(w,c))))&space;&plus;&space;\frac{1}{&hash;(w,u)}\cdot\sum_{u\in&space;W(c)\cap&space;S(w)}sim(w,u)&space;&plus;\frac{1}{&hash;(w,u)}\cdot\sum_{u\in&space;W(c)\cap&space;A(w)}sim(w,v)\right&space;)" target="_blank"><img src="https://latex.codecogs.com/gif.latex?\sum_{w\inV}\sum_{c\inV}\left&space;(&space;\&hash;(w,c)&space;\cdot&space;log(\sigma(sim(w,c)))&plus;k\cdot&space;\&hash;(w\cdot&space;P_0(c)\cdot&space;log(\sigma(-sim(w,c))))&space;&plus;&space;\frac{1}{&hash;(w,u)}\cdot\sum_{u\in&space;W(c)\cap&space;S(w)}sim(w,u)&space;&plus;\frac{1}{&hash;(w,u)}\cdot\sum_{u\in&space;W(c)\cap&space;A(w)}sim(w,v)\right&space;)" title="\sum_{w\inV}\sum_{c\inV}\left ( \#(w,c) \cdot log(\sigma(sim(w,c)))+k\cdot \#(w\cdot P_0(c)\cdot log(\sigma(-sim(w,c)))) + \frac{1}{#(w,u)}\cdot\sum_{u\in W(c)\cap S(w)}sim(w,u) +\frac{1}{#(w,u)}\cdot\sum_{u\in W(c)\cap A(w)}sim(w,v)\right )" /></a>
